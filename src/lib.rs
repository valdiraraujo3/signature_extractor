...
mod integration_test {
    ...
    async fn test_integration() -> Result<(), Box<dyn Error>> {
        dotenv().ok();
        let nats_url =
            env::var("NATS_URL").unwrap_or_else(|_| "nats://192.168.2.107:4222".to_string());
        let nats_subject =
            env::var("NATS_SUBJECT").unwrap_or_else(|_| "pdf.events.started".to_string());
        let stream_name =
            env::var("JS_STREAM_NAME").unwrap_or_else(|_| "PDF_PROCESSING".to_string());
